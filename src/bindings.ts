...
export const commands = {
    async isPathValid(path: string): Promise<Result<null, InvalidPathError>> {
        try {
            return {
                status: "ok",
                data: await TAURI_INVOKE("is_path_valid", { path }),
            };
        } catch (e) {
    ...
    ): Promise<Result<Package[], ResolveError>> {
        try {
            return {
                status: "ok",
                data: await TAURI_INVOKE("resolve_package", { location }),
            };
        } catch (e) {
            if (e instanceof Error) throw e;
            else return { status: "error", error: e as any };
    ...
    async searchRegistry(
        registry: Registry,
        query: string
    ): Promise<Result<SearchResult[], SearchRegistryError>> {
        try {
            return {
                status: "ok",
                data: await TAURI_INVOKE("search_registry", {
                    registry,
                    query,
                }),
            };
...
export type GitPackageLocation = {
    git_type: GitPackageType;
    commit: GitCommit;
};
...
export type PackageLocation =
    ...
    | { url: string }
    | { unknown: { name: string; description: string } };
export type Registry = {
    registry_type: RegistryType;
    custom_url: string | null;
};
export type RegistryPackageLocation = {
    registry: Registry;
    name: string;
    version: string;
};
...
export type ResolveError =
    | { local: LocalResolveError }
    | { git: GitResolveError }
    | { registry: RegistryResolveError };
